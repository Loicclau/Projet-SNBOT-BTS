...
const  char *  ssid  =  "BTS_SN" ; //Galaxy A54 5G
...
//methode pour envoyer HTTP post
void sendPostRequest(String NomText) {

  HTTPClient http;
  // Définir l'URL de destination
  http.begin("http://192.168.1.41:12101/api/text-to-speech");

  // Définir l'en-tête du contenu pour une requête POST
  //http.addHeader("Content-Type", "application/x-www-form-urlencoded");
  http.addHeader("Content-Type", "text/plain");
  // Corps de la requête POST
  //String postData = "param1=valeur1&param2=valeur2";
  String postData = "Bonjour"+  NomText + " je suis le SN Botte, que puige faire pour vous ?";
  ...
  // Si la requête a réussi, réponse du serveur
        ...
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              //appel de la fonction HTTP POST
              sendPostRequest(recognised_message);
            }
            else
            {
